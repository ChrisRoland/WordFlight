...
export default function ChatApp({ userName }: ChatAppProps) {
  ...
  // Optimize: Track app visibility to pause unnecessary listeners
  useEffect(() => {
  ...
  // Optimized: Subscribe to rooms only (lightweight operation)
  useEffect(() => {
    const q = query(collection(db, 'rooms'), orderBy('createdAt', 'asc'));
  ...
  // Optimized: Delete a room and all its messages using batch operations
  const deleteRoom = async (roomId: string) => {
    try {
      ...
      // Add room deletion to the batch
  ...
  return (
    <div className="flex h-screen bg-gray-100 relative">
      ...
      <div className="flex-1 flex flex-col w-full lg:w-auto">
        {currentRoom ? (
          // Only render ChatRoom when app is visible and room is selected
          isAppVisible ? (
            <ChatRoom 
              userName={userName} 
              roomId={currentRoomId}
